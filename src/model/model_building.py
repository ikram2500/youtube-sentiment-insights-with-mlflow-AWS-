...
def apply_tfidf(train_data: pd.DataFrame,  max_features: int, ngram_range: tuple) -> tuple:
    ...
    try:
        ...
        #logger.debug("TF-IDF transformation complete. Train shape:{X_train_tfidf.shape}")
...
def train_lgbm(X_train: np.ndarray, y_train: np.ndarray, learning_rate: float, max_depth: int, n_estimators: int) -> lgb.LGBMClassifier:
    ...
    try:
        ...
        logger.debug('LightGBM model training successfully')
        return best_model
    except Exception as e:
...
def main():
    try:
        ...
        # Load processed training data from the interim directory
        ...
        # Apply TF-IDF feature engineering on the training data
